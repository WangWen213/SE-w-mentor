...
def _ensure_project_owner_schema() -> None:
    with _ENGINE.begin() as connection:
        columns = {row[1] for row in connection.execute(text("PRAGMA table_info(projects)")).all()}
        if "owner_session_hash" not in columns:
            connection.execute(
                text("ALTER TABLE projects ADD COLUMN owner_session_hash VARCHAR(64)")
            )
        indexes = {row[1] for row in connection.execute(text("PRAGMA index_list(projects)")).all()}
